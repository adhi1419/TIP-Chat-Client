...
public class SimpleServer {
    ...
    public static void main(String args[]) throws Exception {
        ...
        while (true) {
            ...
            SimpleClientConnection scc = new SimpleClientConnection(currentId, s);
            scc.start();
...
class SimpleClientConnection extends Thread {
    ...
    public void run() {
        try {
            ...
            try {
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    if(msg.getMessageType().startsWith("PM")){
                        if(!SimpleServer.clients.get((int) mId).contains("***")){
                            String txt = "[" + msg.getTime() + " | " + msg.getIP() + " | " + msg.getMessageType().replace("--", " to ") +"] <"
                        ...
                        if(!SimpleServer.clients.get((int) mId).contains("***")){
                            String txt = "[" + msg.getTime() + " | " + msg.getIP() + "] <" + msg.getNick() + "> " + msg.getMessage();
                            System.out.println(txt);
                            for(SimpleClientConnection a : SimpleServer.clientconnections){
                                a.broadcastMessage(txt);  
                            }
                        }
                    } else if (msg.getMessageType().startsWith("CONTROLMESSAGE")) {
                        String txt = msg.getNick() + "~" + msg.getMessage();
                        System.out.println(SimpleServer.getTimeStamp() + "<ServerView> " + txt.substring(0, txt.indexOf("~"))
                            + " sent the following command: " + txt.substring(txt.indexOf("~") + 1));
                        for(SimpleClientConnection a : SimpleServer.clientconnections){
                ...
                // e.printStackTrace(System.out); //Server has other things to bother about
            }
        } catch (IOException e) {
    ...
    void sendPM(String msg, String msgtype){
        ...
        try {
            ...
            String sender = line.substring(
                    line.indexOf("<") + 1, line.indexOf(">"));
            if(SimpleServer.clients.indexOf(recepient) == mId  || SimpleServer.clients.indexOf(sender) == mId){
                String pm = line.substring(line.indexOf("~") + 1);
                if(!SimpleServer.clients.contains(recepient)){
                    osw.write(SimpleServer.getTimeStamp() + "<***Server***>\n" + recepient + " not found in the Clientlist! :(\nThe" +
    ...
    void sendControlMessage(String msg, String msgtype){
        ...
        try {
            OutputStreamWriter osw = new OutputStreamWriter(
                mSocket.getOutputStream());
            if(SimpleServer.clients.indexOf(line.substring(16, line.indexOf("~"))) == mId){
        ...
        try {
            ...
            if (line.startsWith("<***Server***> !kick")) {
                ...
                if(SimpleServer.clients.indexOf(asshole) == mId){
                    ...
                    System.out.println(SimpleServer.getTimeStamp() + "<***Server***> " + asshole + " was kicked!");
                }
                ...
                if(SimpleServer.clients.indexOf(asshole) == mId){
                    ...
                    osw.flush();
                    SimpleServer.clients.set((int) mId, asshole + "***");
                    System.out.println(SimpleServer.getTimeStamp() + "<***Server***> " + asshole + " was muted!");
                }
                ...
                if(SimpleServer.clients.indexOf(niceguy + "***") == mId){
                    ...
                    System.out.println(SimpleServer.getTimeStamp() + "<***Server***> " + niceguy + " was unmuted!");
                }
            } else {

...
public class SimpleClient {
    ...
    public static void main(String[] args) throws Exception {
        ...
        /* Message Object, which is sent over the Socket, once
        * the Client sets a nick. The Message class contains
        * a Message Parser and also carries a Client's Nick
        * and also status(Online/Offline).
        */
        ...
        while (!((line = s.nextLine()).equals(":quit"))) {
            if(line.equals("//startlog"))
                ...
                log = false;
                SimpleClient.logMessage("You are messages are not being logged now.");
            }
            if(!(line.contains("~") || line.contains("--"))){
                sendMessage(line);     
            } else {
                System.out.println("Message contains Invalid Character(s) (~/--)");
            }
        }
		oos.close();
        s.close();
    ...
    /* This method takes the text the client types,
    *  encapsulates the Message Type, Time of message,
    *  Sender's Nick and the Message itself and sends
    *  it to the server.
    */
